...
**NB**: If after running the dotfiles you are unable to run commands in the command line, comment out the following line in `.bashrc`: `[[ -f "$HOME/.profile" ]] && source "$HOME/.profile"`
